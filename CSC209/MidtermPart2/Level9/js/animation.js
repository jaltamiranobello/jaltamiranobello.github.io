function assignPoints(){
    //assing the arrays to empty arrays to make sure there are no prevoius points set
    points = [];
    ogPoints = [];
    trace = [];
    ...
    for (let i=0; i < nPoints; i++){
        ...
        let ogPointArr = new Array("color", 0, 0, "direction", 0, 0, "speed");
        pointArr[0] = color;
        ...
        ogPointArr[0] = pointArr[0];
        ogPointArr[1] = pointArr[1];
        ogPointArr[2] = pointArr[2];
        ogPointArr[3] = pointArr[3];
        ogPointArr[4] = pointArr[4];
        ogPointArr[5] = pointArr[5];
        ogPointArr[6] = pointArr[6];
        ogPoints.push(ogPointArr);
        let currPoint = new Array();
        trace.push(currPoint);
        let firstPos = new Array(pointArr[1], pointArr[2]);
        trace[i].push(firstPos);
    }
    createPoints();
}
...
function startAnimation(){
    ...
    clearInterval(intervalID);
    //call the animate points every 20 milisecond until NRSTEPS steps are executed
...
//format [color, circle X-coordinate, circle Y-coordinate, direction, end of line X-coordinate, end of line Y-coordinate, speed]
    ...
    let nextPos = new Array(points[idx][1], points[idx][2]);
    trace[idx].push(nextPos);
}
...
// for each item in points get the corodinates and color to draw them 
    ...
    for(let i = 0; i < points.length; i++){
        ...
        ctx.arc(points[i][1], points[i][2], 20, 0, 2 * Math.PI);
        ctx.stroke();
        // check to draw traces or not
        let checkbox = document.getElementById("optTrace");
        if (checkbox.checked == true){
            //draw all the traces
            for(let j= 0; j < trace[i].length; j++){
                ctx.beginPath();
                ctx.arc(trace[i][j][0], trace[i][j][1], 5, 0, 2 * Math.PI);
                ctx.stroke();
            }
        }
        //other wise only draw the current point and the vector
        //draw the vector
        ctx.beginPath();
        ctx.moveTo(points[i][1], points[i][2]);
        ctx.lineTo(points[i][4], points[i][5]);
...
function resetPoints(){
    ...
    const ctx = canvas.getContext("2d");
    ctx.lineWidth = 5;
    //clear the canvas to reset the points
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    //reset the trace array so that when draw is called it can not pre-draw the trace path
    trace = [];
    for (let i=0; i < ogPoints.length; i++){
        //draw the og circle
        ctx.beginPath();
        ctx.strokeStyle = ogPoints[i][0];
        ctx.arc(ogPoints[i][1], ogPoints[i][2], 20, 0, 2 * Math.PI)
        ctx.stroke();
        //draw the og vector
        ctx.beginPath();
        ctx.moveTo(ogPoints[i][1], ogPoints[i][2]);
        ctx.lineTo(ogPoints[i][4], ogPoints[i][5]);
        ctx.stroke();
        //update the global point so the animation can move from the og points
        points[i][0] = ogPoints[i][0];
        points[i][1] = ogPoints[i][1];
        points[i][2] = ogPoints[i][2];
        points[i][3] = ogPoints[i][3];
        points[i][4] = ogPoints[i][4];
        points[i][5] = ogPoints[i][5];
        points[i][6] = ogPoints[i][6];
        //reset the trace array so that when draw is called it can not pre-draw the trace path
        let currPoint = new Array();
        trace.push(currPoint);
        let firstPos = new Array(ogPoints[i][1], ogPoints[i][2]);
        trace[i].push(firstPos)
    } 
}
...
<?php
    ...
    function findUserId(array $usersInfo, string $userName): int{
        $totalUsers = count($usersInfo);
        //start at index 1 because index 0 will always be admin
        for ($i=1; $i < $totalUsers; $i++){
            $currUser = $usersInfo[$i];
    ...
    function updateFileUserDeletion(array $usersInfo, string $filePath){
        $myfile = fopen($filePath, "w");
        $updatedContent = json_encode($usersInfo);
        fwrite($myfile, $updatedContent);
        fclose($myfile);
    ...
    function userExists(array $usersInfo, string $username){
        $exists = false;
        $totalUsers = count($usersInfo);
        //start at index 1 because index 0 will always be admin
        for ($i=1; $i < $totalUsers; $i++){

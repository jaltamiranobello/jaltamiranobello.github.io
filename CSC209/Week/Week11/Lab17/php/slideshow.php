...
<?php
    function createSlides(array $imgFilePaths, int $imgNR){
        ...
        for ($i = 0; $i < $imgNR; $i++){
            ...
            $extensionIdx = strrpos($imgFilePath, '.');
            $extensionLength = strlen($imgFilePath) - $extensionIdx;
            $length = strlen($imgFilePath) - $startIdx - $extensionLength; // subtract the unnecessary characters before the file name and the extension
